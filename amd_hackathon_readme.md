...
AMD Developer Hackathon: ACT II AI Hackathon | lablab.ai
...
Online
Monday, July 6 2026 - 9:00 AM Pacific Daylight Time
...
New sign-ups only
☁️ $100 AMD Developer Cloud GPU Credits
...
⭐ Beginner · AI Agent Track
Track 2
Video Captioning
🎬 Beginner · Prompt and have fun
Track 3
Unicorn Track
...
Beginner
Judging
LLM-Judge for accuracy and tone
Compute
Fireworks AI API
Track 3
🦄 All Levels
...
Level
All skill levels and any tech stack
Judging
Creativity, originality, completeness, use of AMD platforms, product/market potential
Compute
AMD GPUs + Fireworks AI API
Technology & Access
...
Start Building →
Native.Builder is an AI-native environment for building software, workflows, and agents fast. For this hackathon, participants can use Fireworks AI credits inside Builder and tap into the models available through the platform to go from concept to working prototype quickly.
...
Through Native.Builder using Fireworks AI credits

Models

Models available inside Builder

Compute

Builder workspace with integrated Fireworks-powered model access

Sign-up

Through Native.Builder event onboarding

Native.Builder Website
Documentation
Getting Started Guide
💡
Quick Note on Builder Access
Native.Builder is available as a supported technology option for this hackathon. Participants can use the Fireworks AI credits provided for the event inside Builder. Model availability may vary based on Builder configuration and track requirements.

🚀
How to Get Started with Native.Builder
Open Native.Builder and start a new project.
Use your Fireworks AI credits through Builder Integrations for model access.
...
$1,000
Track 2
Best Use of Gemma in Video Captioning
$3,000
Track 3
...
The startup or product vision — how compelling and viable the idea is in a real market context.
...
Event Schedule
Jul 6 9:00 AM Pacific Daylight Time
Hackathon Kick-off

Jul 6 9:05 AM Pacific Daylight Time
lablab.ai opening words
Pawel Czech - NativelyAI CEO & lablab.ai Founder

Jul 6 9:10 AM Pacific Daylight Time
AMD opening words
Ramine Rosen - AI Corporate VP

Jul 6 9:15 AM Pacific Daylight Time
NativelyAI opening words
Adam V. Mlady - DevRel

Jul 6 9:20 AM Pacific Daylight Time
Google opening words
Ian Ballantyne - Developer Relations Engineer

Jul 6 9:25 AM Pacific Daylight Time
Introduction to the Challenge

Jul 6 9:35 AM Pacific Daylight Time
Hackathon Guide

Jul 6 10:00 AM Pacific Daylight Time
Discord Q&A session

Jul 11 9:00 AM Pacific Daylight Time
End of Submissions!
...
Neuroloom — Family Care Command Center
Neuroloom is a multi-agent family care command center. Nine Gemma-powered agents on AMD GPUs coordinate meds, handoffs, documents, check-ins, and emergencies — one live dashboard for 53M+ caregivers.
...
Play
SignalSniper AI: Forex Intelligence Platform
SignalSniper AI: Forex Intelligence Platform
SignalSniper AI is an explainable Forex intelligence platform that uses AI to analyze technical indicators and transform complex market signals into clear BUY, SELL, or HOLD insights with human-readable reasoning.
...
Crucible — the autonomous AI underwriter
The autonomous AI underwriter that puts loans on trial, works the conditions, and rebuilds denials — every claim cited to real guideline text, running on AMD Instinct MI300X.
...
TokenForge — Hybrid Token-Efficient Routing Agent
TokenForge is an intelligent routing agent that sends each query down the cheapest correct path — solving simple math and greetings locally for free, and escalating only complex reasoning to Fireworks AI to maximize accuracy while minimizing token spend.

Multimodal TokenForge
AgentOps
Yi-LLMS
...
OmniSolve
OmniSolve
OmniSolve is a high-performance AI agent built for the AMD Hackathon. It leverages Fireworks AI to efficiently tackle tasks across 8 diverse domains—including math, coding, and logical reasoning—optimizing for both accuracy and token usage.
...
Token-Efficient Agent
Answers the Track 1 tasks using the fewest Fireworks tokens.

ND
AgentOps

Play
CineScribe: Style-Conditioned Captioning Agent
CineScribe: Style-Conditioned Captioning Agent
...
Play
RouteZero — Token-Efficient Task Agent
RouteZero — Token-Efficient Task Agent
A Track 1 routing agent that completes all eight task categories while spending the fewest tokens possible, deciding per task, in real time, how little inference each answer actually needs.
...
Play
Agentic EWS Advisory system 
Agentic EWS Advisory system
the app will help Smallholder farmers in agricultural zonesget seasonal forecast and advisories to reduce planting-window errors, crop failure, and increase uptake of insurance cover owing to emergence of climatic regimes. advisory tools
...
VoxRouter — Hybrid Token-Efficient Routing Agent
An AI router that sends each task to the cheapest model that can handle it — free local models via Ollama for simple tasks, remote models only when complexity demands it, with confidence-based escalation and a built-in benchmark score.
...
Fusion Net
Fusion Net
FusionNet is an AMD-optimized federated AI platform enabling edge devices to collaboratively fine-tune LLMs without sharing raw data using LoRA and differential privacy. We don't move data to the model. We move the model to the data.
...
Play
TokenOptimizer — Hybrid Routing Agent
TokenOptimizer — Hybrid Routing Agent
A hybrid routing agent that answers cheap tasks for free on a local model with deterministic solvers, and escalates only the genuinely hard ones to Fireworks AI — clearing the accuracy gate with the fewest possible tokens across all 8 categories.
...
CUDA to ROCm Kernel Parity Check
AMP proves a ported CUDA kernel produces matching output on real AMD Instinct MI300X hardware, turning ROCm migration from an open-ended guessing game into a same-day, evidence-backed decision.
...
Axiom - Aether
Axiom - Aether
Axiom is a local-first Rust research runtime for online Test-Time Training (TTT), context compression, grounding checks, and self-healing execution loops.
...
Play
Token Sage Application 
Token Sage Application
TokenSage is a hybrid AI routing system that intelligently directs simple queries to a local AMD Ryzen AI NPU and complex queries to cloud LLMs, reducing token usage, lowering costs, improving response speed, and enhancing user privacy.
...
Play
FairPrice: AI Agents for Africa's Informal Economy
FairPrice: AI Agents for Africa's Informal Economy
An AI-powered escrow marketplace solving cash-on-delivery risk for Africa's informal sellers; autonomous agents (ZEMA 360) run fulfillment and pricing on Qwen 3.7 Plus, served on AMD Instinct GPUs via Fireworks AI, live in production today.
...
Play
Aegisflow Security dashboard 
Aegisflow Security dashboard
AegisFlow: real-time network security for banking SOC teams. Live packet capture, AI-driven threat triage, and tamper-proof forensic evidence — built by a student, engineered like production.

AegisFlow Network Security Dashboard
application badge
AMD Developer Cloud
AMD ROCm
...
Scaffold — Compound AI for Product Photography
Compound AI system for product photography. Per-scene routing between a compose path (guaranteed preservation, 20× cheaper) and an edit path (FLUX Kontext), Kimi K2.6 verification on Fireworks, self-healing retry. Live telemetry.
...
Play
Frugal Router, a Token-Efficient AI Agent
Frugal Router, a Token-Efficient AI Agent
A general-purpose AI agent that completes eight task categories using the fewest remote tokens. A free local model gauges its own confidence and shapes each Fireworks call to be as cheap as possible while staying above the accuracy gate.
...
DeliveryBoy AI 
DeliveryBoy AI
DeliveryBoy AI is an intelligent assistant for motorcycle couriers that optimizes routes, predicts demand, improves productivity, and increases road safety using AMD AI technologies and edge computing.
...
Hybrid Token-Efficient Routing Agent. (Track 1)
An AI agent routing tasks across two tiers to minimize Fireworks API tokens. Simple tasks run locally for free via Ollama, while complex tasks (math, code, logic) are sent to Fireworks AI. AMD Developer Hackathon ACT II Track 1.
...
Stryvo Vision
Stryvo Vision
Containerized AI agent that watches video clips and generates captions in four distinct styles (formal, sarcastic, humorous-tech, humorous-non-tech) using Fireworks AI VLMs.
...
TERA: Token-Efficient Routing Agent
TERA is high-accuracy, general-purpose AI agent that utilizes a hybrid routing architecture. By combining dynamic category budgeting, prompt compression, semantic caching, and 0-token rule engine, TERA solves complex tasks with extreme token efficiency.
...
hybrid-routing-agent
A hybrid AI agent that routes every query to the cheapest capable model — free local inference first, Fireworks AI only when the task actually needs it — to maximize accuracy per token spent.

Jetpack
Claude Code

Play
AfroAi - Cake Pricing Agent for Nigerian Bakers
AfroAi - Cake Pricing Agent for Nigerian Bakers
AI that calculates exact ingredient costs and profit for Lagos bakers. Input prices, get cost per cake + recommended selling price. No GPU needed.
...
Initial Submission
Initial Submission
A token-efficient general-purpose AI agent that uses rule-based task routing across 8 capability categories — factual knowledge, math, sentiment, summarization, NER, code debugging, logical reasoning, and code generation — powered by Fireworks AI.
...
Hybrid Token-Efficient Routing Agent
Hybrid Token-Efficient Routing Agent
A token-efficient, general-purpose AI agent for AMD Hackathon Track 1. It classifies each task into one of eight categories with cheap regex routing, then answers with the smallest Fireworks model that clears the accuracy gate—minimizing total tokens.

T_ying_yai
AgentOps

Play
Armenian ASR: State-of-the-Art Speech Recognition
Armenian ASR: State-of-the-Art Speech Recognition
Building a state-of-the-art automatic speech recognition system for Armenian using one of the largest gated Armenian speech datasets with over 8,000 hours of audio. Our goal is to make high-quality speech AI accessible for a low-resource language.
...
SplitLLM – Explainable Hybrid AI Routing
SplitLLM is an explainable hybrid AI routing platform that analyzes every prompt before execution, intelligently selecting the best local or cloud model to reduce cost, improve latency, and optimize AI inference with transparent reasoning.
...
VIDEO CAPTIONING 
VIDEO CAPTIONING
Models are accessed via Fireworks AI API credits. Fine-tuning is explicitly permitted — you may also train your own captioner and use it alongside or instead of prompting. Use open datasets, build your own, or both. used will be revealed on launch day.

team lablab winners
AI Studio

Play
ApexFlow AI: Self-Healing Telemetry Gateway
ApexFlow AI: Self-Healing Telemetry Gateway
ApexFlow AI is a self-healing edge telemetry gateway that uses a hybrid quantum-classical VQC router to autonomously reconcile schema drift in real-time, preventing mission-critical IoT data blackouts on AMD Instinct GPUs.
...
Video Captioning Agent
An agent that watches a video clip, samples frames with ffmpeg, and generates faithful captions in four required styles: formal, sarcastic, humorous tech, and humorous non-tech.
...
ClipForger-AMD
ClipForger-AMD
ClipForger-AMD turns short videos into accurate burned-in captions plus four styled summaries: formal, sarcastic, humorous-tech, and humorous-non-tech, using Fireworks/Gemma-first routing with fallback reliability.
...
GymHub Women
GymHub Women
GymHub Women is an AI-powered women-only fitness capsule network for Dubai/MENA. Powered by AMD MI300X + LLaMA 3.1 70B via Fireworks AI, our smart coach adapts workouts to hormonal cycles, biometrics, and personal goals in real time.
...
FireRoute Agent
FireRoute Agent
FireRoute AI is a Dockerized Track 1 agent that reads hidden natural-language tasks, routes them by capability, calls Fireworks AI through the harness-provided environment and writes valid JSON answers with an accuracy-first, token-aware strategy.
...
Hybrid Token-Efficient Routing Agent
Hybrid Token-Efficient Routing Agent
A FastAPI hybrid routing agent that optimizes token costs. It routes simple tasks to local Ollama, while seamlessly upgrading complex requests to Gemini 2.5 Flash for high-accuracy reasoning with automatic error failover.

Team dev
Gemini AI

Play
The Shield-17799: Prevent Before Harm
The Shield-17799: Prevent Before Harm
The Shield-17799 is an AI-powered investigative intelligence platform that detects online grooming and high-risk behavior before harm occurs, combining criminal investigation expertise with AI to support early intervention and child protection.

The shield-17799 ai
AI/ML API

Play
Code Time Capsule/Project
Code Time Capsule/Project
Code Time Capsule transforms your repository’s commit history into an interactive timeline with AI-powered answers about any moment in your codebase’s past.
...
FACTOR (Factual Agentic Content Orchestrator)
Self-hosted AI agentic pipeline designed to autonomously produce high-quality, hallucination-free content. Built to prioritize factual integrity, FACTOR generates articles, reviews, tutorials, and opinion
...
Play
Teams: AMD Developer Hackathon: ACT II
Check out the roster and find teams to join

Neural Nova
Neural Nova

From algorithms to impact—creating AI that matters.

team member
shreyyy_1
neoLOOKING FOR MEMBERS
neo

will make a finance saas
...
A gap that can be filled by Solution

team member
Aozora
Aozora

AI for you, for me and for everyone
...
https://nativelyai.com
https://surge.lablab.ai/
Links
AI Tech
AI Hackathons
AI Tutorials
...
# RL Fine-Tuning Project Ideas (Our Analysis — July 7, 2026)

Deadline: **July 11, 9:00 AM PDT** (~4 days). Resources: AMD Developer Cloud credits
(MI300X-class GPUs, 192GB HBM), Fireworks AI API credits, $6,000 Gemma side-prize pool.
Fine-tuning is explicitly permitted in Tracks 1 and 2.

## Lessons from last hackathon's winning projects

**No winner used RL fine-tuning** — that's our differentiator. But they won with strong
engineering patterns that we must build on, not replace:

- **Track 1 winners** (TokenForge, TERA, VoxRouter, TokenOptimizer, Frugal Router):
  - Zero-token paths first — deterministic solvers (regex, calculators, rule engines) answer simple tasks for free
  - Free local model as second tier (only Fireworks tokens are scored)
  - Confidence-based escalation — local model gauges certainty, escalates only when unsure
  - Token-shaping tricks — prompt compression, semantic caching, per-category budgets
- **Track 2 winners** (CineScribe, ClipForger): multimodal perception stack (frame timeline
  + Whisper audio transcript) is table stakes; fallback reliability so the demo never breaks
- **Track 3 winners** (Neuroloom, Crucible, FairPrice, Scaffold, FusionNet): production
  feel — live telemetry, real users, self-healing loops, clear market narrative

## Idea 1 (RECOMMENDED): RLAIF-tuned Gemma captioner — Track 2

Track 2 is judged by an LLM judge on accuracy and tone across four styles (formal,
sarcastic, humorous-tech, humorous-non-tech). RLAIF trains against the same kind of
signal we're scored on.

**Architecture:**
- Frame sampling + audio transcript (ffmpeg + Whisper) → visual/audio context (CineScribe pattern)
- Fine-tune **Gemma 3 4B/12B** (multimodal) with **GRPO** on MI300X: generate N candidate
  captions per clip per style, score each with a strong LLM judge (via Fireworks) on
  faithfulness + style adherence, use scores as group-relative rewards
- Keep a prompted Fireworks fallback for reliability (ClipForger pattern)

**Why it wins:** nearly everyone prompts a VLM; an RL-tuned model that reliably separates
*sarcastic* from *humorous-tech* tone is a real differentiator. Prize stacking: Track 2
placement ($1,000–$2,500) + **Best Use of Gemma in Video Captioning ($3,000 — largest
single partner prize)** + meaningful AMD GPU use for training. TRL's GRPOTrainer runs on
ROCm PyTorch; LoRA GRPO on 4B–12B fits in one MI300X and the credit budget.

**4-day plan:**
- Day 1: data pipeline (clips → frames/transcripts) + prompted baseline producing all four
  styles end-to-end, containerized — *this alone is a valid submission (de-risk)*
- Day 2: judge/reward design + SFT warm-start on judge-filtered outputs
- Day 3: GRPO run on MI300X + side-by-side eval vs baseline
- Day 4: demo video, README, submission

## Idea 2: RL-trained escalation policy — Track 1

Don't RL-train a model *picker* — RL-train the **escalation policy over the whole winning
architecture**. "TERA's architecture, but the routing thresholds are learned, not guessed."

- Action space: {deterministic solver, free local Gemma, Fireworks-tiny, Fireworks-mid,
  Fireworks-large} + prompt-compression level
- Reward: correctness − λ · Fireworks tokens (local eval harness over the 8 task categories:
  factual QA, math, sentiment, summarization, NER, code debugging, logic, codegen)
- Router runs locally (free — local inference isn't scored); only routed Fireworks calls cost

**Why it could win:** objective leaderboard; a learned policy can discover non-obvious
routings that beat hand-tuned heuristics. Also eligible for the Track 1 Gemma prize ($1,000).
**Risk:** most crowded track; betting a learned policy beats a season of hand-tuned rules.

## Idea 3: "GRPO-on-ROCm in a box" — Track 3 (Unicorn)

A containerized, one-command RL fine-tuning stack for AMD GPUs: pick a base model (Gemma),
define a reward function (Python or LLM-judge), run GRPO/DPO on MI300X with live reward
curves, and get a vLLM-ROCm serving endpoint. "Unsloth/verl ergonomics, but AMD-native."

**Why it could win:** maximal "use of AMD platforms"; credible startup story against the
CUDA monoculture (FusionNet won with federated LoRA on AMD — this is a sibling). Eligible
for Best AMD-Hosted Gemma Project ($2,000).
**Risk:** highest — needs Scaffold-level polish (telemetry, one-command demo) in 4 days;
subjective judging.

## Recommendation

**Idea 1 (Track 2 RLAIF captioner)**, built on proven perception + fallback patterns, with
the prompted pipeline as the day-1 safe submission and the RL-tuned model as strict upside.
Track 1 is the fallback if we prefer an objective leaderboard. Track 3 is highest-risk.